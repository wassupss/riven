...
final class GitPanel: NSView, Themable, Scalable, NSTextViewDelegate {
    ...
    private func remoteOp(_ name: String, _ op: @escaping (String) -> (ok: Bool, error: String)) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                if !res.ok {
                    self.alert(t("git.opFailed", ["name": name]), res.error.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                        ? t("git.remoteFailBody") : res.error)
                }
                self.refresh()
            }
        }
    ...
    @objc private func commit() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                if res.ok { self.commitMsg.string = "" } else { self.alert(t("git.commitFailed"), res.error) }
                self.refresh()
            }
        }
    }
    ...
    private func discard(_ f: Git.GitFile) {
        ...
        let a = NSAlert(); a.messageText = t("git.discardConfirm", ["name": name])
        a.informativeText = t("git.discardBody"); a.addButton(withTitle: t("git.discardBtn")); a.addButton(withTitle: t("common.cancel"))
